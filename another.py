...
class DataloaderReranking():
    ...
    def tokenizers(self, batch_question:List, batch_context:List):
        tokenize_context = self._tokenizer.batch_encode_plus(
            #batch_question,
            #batch_context,
            batch_text_or_text_pairs=zip(batch_question, batch_context),
            max_length=self.config.max_length,
            padding='max_length',
            return_token_type_ids=True,
            return_attention_mask =True,
    ...
    def fit_context(self):
        ...
        self.tokenize_context = self.tokenizer(self.clean_context)
